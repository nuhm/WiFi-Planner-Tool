...
export const deleteNodeLogic = ({
   ...
   setNodes(prevNodes =>
      prevNodes.filter(node => Math.hypot(node.x - snappedPos.x, node.y - snappedPos.y) > NODE_DISTANCE_THRESHOLD)
   );

   setWalls(prevWalls =>
      prevWalls.filter(({ a, b }) => {
         const matchesA = Math.abs(a.x - snappedPos.x) < WALL_MATCH_THRESHOLD && Math.abs(a.y - snappedPos.y) < WALL_MATCH_THRESHOLD;
         const matchesB = Math.abs(b.x - snappedPos.x) < WALL_MATCH_THRESHOLD && Math.abs(b.y - snappedPos.y) < WALL_MATCH_THRESHOLD;
...
export const addAPLogic = ({
   ...
   const collidesWithAP = accessPoints.some(ap =>
      Math.hypot(ap.x - snappedPos.x, ap.y - snappedPos.y) < AP_DISTANCE_THRESHOLD
   );
   if (collidesWithAP) {
   ...
   const collidesWithWall = walls.some(({ a, b }) =>
      distanceToSegment(snappedPos, a, b) < WALL_MATCH_THRESHOLD
   );
   if (collidesWithWall) {
 ...
 */
function getSnappedCursorPos(event, canvasRef, offset, zoom, snapToGrid, setCursorPos) {
   const rect = canvasRef.current.getBoundingClientRect();
   const x = (event.clientX - rect.left - canvasRef.current.width / 2 - offset.x) / zoom;
   const y = (event.clientY - rect.top - canvasRef.current.height / 2 - offset.y) / zoom;
